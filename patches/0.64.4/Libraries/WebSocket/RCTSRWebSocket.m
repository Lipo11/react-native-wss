...
- (instancetype)initWithURLRequest:(NSURLRequest *)request protocols:(NSArray<NSString *> *)protocols options:(NSDictionary *)options
{
...
{
  return [self initWithURLRequest:request protocols:protocols options:nil];
}
...
- (instancetype)initWithURL:(NSURL *)URL protocols:(NSArray<NSString *> *)protocols options:(NSDictionary *)options
{
...
- (void)_initializeStreams
{
  ...
  if (_secure) {
    ...
    if (_urlRequest.RCTSR_SSLPinnedCertificates.count || ( _options != nil && [_options objectForKey:@"ca" ] != nil ) ) {
      [SSLOptions setValue:@NO forKey:(__bridge id)kCFStreamSSLValidatesCertificateChain];
    }
    
    if( _options != nil && [_options objectForKey:@"pfx" ] != nil && ![[_options objectForKey:@"pfx"] isEqualToString:@""] )
    {
      [SSLOptions setValue:@NO forKey:(__bridge id)kCFStreamSSLValidatesCertificateChain];
      
      NSData * _clientCertificateData = [[NSData alloc] initWithBase64EncodedString:[_options objectForKey:@"pfx"] options:0];
      
      CFArrayRef keyref = NULL;
      OSStatus status = SecPKCS12Import((__bridge CFDataRef)_clientCertificateData,
                                        (__bridge CFDictionaryRef)[NSDictionary
                                                                   dictionaryWithObject:[_options objectForKey:@"passphrase"]
      ...
      if (status == noErr)
      {
        ...
        if( !status )
        {
          ...
          NSArray *tokenCert = [[NSArray alloc] initWithObjects:(__bridge id)identityRef, (__bridge id)cert, nil];
          
          [SSLOptions setObject:tokenCert forKey:(NSString *)kCFStreamSSLCertificates];
        }
      }
...
{
   if (_secure && !_pinnedCertFound && _options != nil && [_options objectForKey:@"ca"] != nil && ![[_options objectForKey:@"ca"] isEqualToString:@""] && ( eventCode == NSStreamEventHasBytesAvailable || eventCode == NSStreamEventHasSpaceAvailable ) )
    {
      ...
      //Create a bland policy which only checks signature paths.
      OSStatus err = SecTrustCreateWithCertificates((__bridge CFArrayRef)(chain), SecPolicyCreateBasicX509(), &trust);
      
      if( err == errSecSuccess )
      {
        NSArray * splitedCa = [[_options objectForKey:@"ca"] componentsSeparatedByString:@"-----BEGIN CERTIFICATE-----"];
        NSMutableArray * optionCerts = [NSMutableArray array];
        
        for( NSString * cert in splitedCa )
        {
          NSString * certNoSpaces = [cert stringByReplacingOccurrencesOfString:@"\n" withString:@""];
          
          if( [certNoSpaces length] > 0 )
          {
            NSData * rawCertificate = [[NSData alloc] initWithBase64EncodedString:[certNoSpaces stringByReplacingOccurrencesOfString:@"-----END CERTIFICATE-----" withString:@""] options:NSDataBase64DecodingIgnoreUnknownCharacters];
            SecCertificateRef anchor = SecCertificateCreateWithData(kCFAllocatorDefault, (__bridge CFDataRef)rawCertificate);
            
            [optionCerts addObject:(__bridge id) anchor];
          }
        }
        
        err = SecTrustSetAnchorCertificates(trust, (__bridge CFArrayRef)(optionCerts));
    ...
    else if (_secure && !_pinnedCertFound && (eventCode == NSStreamEventHasBytesAvailable || eventCode == NSStreamEventHasSpaceAvailable))
    {
